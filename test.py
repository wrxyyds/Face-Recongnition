...
# 优化的训练函数
def train(cls, freeze_ratio=0.7, use_early_stopping=True, experiment_name=None):
    """
    训练模型函数

    Args:
        freeze_ratio (float): 冻结模型层的比例
        use_early_stopping (bool): 是否使用早停策略
        experiment_name (str): 实验名称，用于TensorBoard日志区分
    """
    img_path = './images/train/'
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"使用设备: {device}")

    # 如果没有提供实验名称，则根据早停状态创建名称
    if experiment_name is None:
        experiment_name = f"{'with_early_stopping' if use_early_stopping else 'without_early_stopping'}"

    # 设置TensorBoard日志目录，添加实验名称使不同实验的日志分开存储
    log_dir = f'./logs/tensorboard/{experiment_name}'
    ...
    for epoch in range(num_epochs):
        ...
        if epoch_loss < best_loss:
            ...
            if use_early_stopping and patience_counter >= patience:
                ...
                # TensorBoard: 记录早停事件
                writer.add_text('Early_Stopping',
                                f'Training stopped early at epoch {epoch + 1} for class {cls}',
                                epoch)
                break
...
if __name__ == "__main__":
    # 运行不同的训练实验以比较早停的影响
    # 使用早停训练
    train('2107090712', freeze_ratio=0.7, use_early_stopping=True, experiment_name="with_early_stopping")

    # 不使用早停训练（可选，取消注释下面一行来进行比较）
    # train(freeze_ratio=0.7, use_early_stopping=False, experiment_name="without_early_stopping")